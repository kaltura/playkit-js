  ...
  _createClass(Player, [{
    ...
    value: function _attachMedia() {
      ...
      if (this._engine) {
        ...
        this._eventManager.listen(this._engine, _events.HTML5_EVENTS.ERROR, function (event) {
          return _this6.dispatchEvent(event);
        });
        ...
        this._eventManager.listen(this._engine, _events.CUSTOM_EVENTS.STREAM_METADATA, function (event) {
          _this6.dispatchEvent(event);
        });
      }
    }
...
var Html5 = function (_FakeEventTarget) {
  ...
  _createClass(Html5, [{
    ...
    value: function attach() {
      ...
      Object.keys(_events.HTML5_EVENTS).forEach(function (html5Event) {
        _this2._eventManager.listen(_this2._el, _events.HTML5_EVENTS[html5Event], function () {
          if (_events.HTML5_EVENTS[html5Event] == _events.HTML5_EVENTS.PLAYING && !_this2._registeredMetadataEvent && _this2._config.playback.registerMetadataTrackEvent) {
            setTimeout(function () {
              _this2._registerMetadata();
            }, 1000);
          }
    ...
    key: '_registerMetadata',
    value: function _registerMetadata() {
      var _this7 = this;

      var tracks = this._el.textTracks;
      for (var track in tracks) {
        if (tracks.hasOwnProperty(track)) {
          if (tracks[track].kind == "metadata") {
            tracks[track].mode = "showing";
            this._registeredMetadataEvent = true;
            tracks[track].addEventListener("cuechange", function (evt) {
              var activeCues = [];
              for (var cue in evt.currentTarget.activeCues) {
                if (evt.currentTarget.activeCues.hasOwnProperty(cue)) {
                  var currentCue = evt.currentTarget.activeCues[cue];
                  activeCues.push(currentCue);
                }
              }
              _this7.dispatchEvent(new _fakeEvent2.default(_events.CUSTOM_EVENTS.STREAM_METADATA, { activeCues: activeCues }));
            });
          }
        }
      }
...
var NativeAdapter = function (_BaseMediaSourceAdapt) {
  ...
  _createClass(NativeAdapter, [{
    ...
    value: function _onError(reject, error) {
      reject(new _error2.default(_error2.default.Severity.CRITICAL, _error2.default.Category.PLAYER, _error2.default.Code.NATIVE_ADAPTER_LOAD_FAILED, error.payload));
    }
...
var _severity = __webpack_require__(52);

var _code = __webpack_require__(53);
