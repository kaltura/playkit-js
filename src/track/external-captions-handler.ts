...
import PKTextTrack from './playkit-text-track';
...
import {PKVTTCue} from './playkit-vtt-cue';
...
class ExternalCaptionsHandler extends FakeEventTarget {
   ...
   * @private
   */
  _activeTextCues: Array<PKVTTCue> = [];
  /**
   ...
   * @public
   */
  getExternalTracks(tracks: Array<PKTrack>): Array<PKTextTrack> {
   ...
   * @private
   */
  _maybeAddTrack(track: PKTextTrack, caption: PKExternalCaptionObject, playerTextTracks: Array<PKTrack>, newTextTracks: Array<PKTextTrack>): void {
    const sameLangTrack = playerTextTracks.find(textTrack => textTrack.available && PKTrack.langComparer(caption.language, textTrack.language));
    if (!sameLangTrack) {
   ...
   * @private
   */
  _createTextTrack(caption: PKExternalCaptionObject): PKTextTrack {
    return new PKTextTrack({
  ...
  _selectTextTrack(textTrack: PKTextTrack) {
    this.hideTextTrack();
    if (this._player.config.text.useNativeTextTrack) {
   ...
   * @private
   */
  _getCuesString(textTrack: PKTextTrack): Promise<any> {
   ...
   * @private
   */
  _downloadAndParseCues(textTrack: PKTextTrack): Promise<void> {
   ...
   * @private
   */
  _handleCaptionOnTimeUpdate(track: PKTextTrack): void {
   ...
   * @private
   */
  _maybeAddToActiveCues(track: PKTextTrack): boolean {
    const currentTime = this._player.currentTime;
    if (!currentTime) {
      return false;
    }
  ...
  _resetExternalNativeTextTrack(): void {
    ...
    if (videoElement && videoElement.textTracks) {
      const track: TextTrack | undefined = Array.from(videoElement.textTracks).find(
        (track): track is TextTrack => track ? PKTextTrack.isExternalTrack(track) : false
      );
      if (track) {
   ...
   * @param {Array<PKVTTCue>} cues - the cues to be added
   * @return {void}
   */
  _addCuesToNativeTextTrack(cues: Array<VTTCue>): void {
    ...
    if (videoElement && videoElement.textTracks) {
      ...
      if (track) {
        track.mode = PKTextTrack.MODE.SHOWING;
  ...
  _addNativeTextTrack(): void {
    const videoElement = this._player.getVideoElement();
    if (videoElement && videoElement.textTracks) {
      const sameLanguageTrackIndex = Array.from(videoElement.textTracks).findIndex(track => (track ? PKTextTrack.isExternalTrack(track) : false));
      if (sameLanguageTrackIndex > -1) {
        this._resetExternalNativeTextTrack();
      } else {
        videoElement.addTextTrack(PKTextTrack.KIND.SUBTITLES, PKTextTrack.EXTERNAL_TRACK_ID, PKTextTrack.EXTERNAL_TRACK_ID);
      }
    }
  }
   ...
   * @private
   */
  _setTextTrack(textTrack: PKTextTrack): void {
    if (!this._player.config.text.useNativeTextTrack) {
