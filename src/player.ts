...
import {StreamType} from './engine/stream-type';
import {EngineType} from './engine/engine-type';
import {MediaType} from './enums/media-type';
import {AbrMode} from './track/abr-mode-type';
import {CorsType} from './engine/html5/cors-types';
import PlaybackMiddleware from './middleware/playback-middleware';
import {DefaultConfig, DefaultSources} from './player-config';
...
import {EngineDecorator, EngineDecoratorType} from './engine/engine-decorator';
...
import {EngineDecoratorManager} from './engine/engine-decorator-manager';
...
export default class Player extends FakeEventTarget {
   ...
   * @private
   */
  _tracks!: Array<PKTrack | PKTextTrack | PKAudioTrack | PKVideoTrack>;
  /**
   ...
   * @public
   */
  getTracks<T extends PKTrack | PKAudioTrack | TextTrack | PKVideoTrack | PKImageTrack>(type?: TrackTypes): T[] {
    switch (type) {
   ...
   */
  getActiveTracks(): {video: PKVideoTrack, audio: PKAudioTrack, text: PKTextTrack} {
  ...
  selectTrack(track?: PKTrack): void {
    if (this._engine) {
      if (track instanceof PKVideoTrack) {
      ...
      } else if (track instanceof PKAudioTrack) {
  ...
  _applyABRRestriction(config: any): void {
    if (Utils.Object.hasPropertyPath(config, 'abr.restrictions') && this._engine && this._tracks.length) {
      ...
      const videoTracks: PKVideoTrack[] = <PKVideoTrack[]>this._tracks.filter(track => track instanceof PKVideoTrack);
      const newVideoTracks = filterTracksByRestriction(videoTracks, restrictions);
      if (newVideoTracks.length) {
        const currentVideoTracks = this._tracks.filter(track => track instanceof PKVideoTrack && track.available);
        const tracksHasChanged = !(
          currentVideoTracks.length === newVideoTracks.length &&
          currentVideoTracks.every((element: PKVideoTrack, index: number) => {
        ...
        if (tracksHasChanged) {
          ...
          this._tracks.forEach(track => {
            if (newVideoTracks.includes(<PKVideoTrack>track) || !(track instanceof PKVideoTrack)) {
   ...
   * @private
   */
  _getTracksByType<T extends TextTrack | PKAudioTrack | PKVideoTrack | PKImageTrack>(type: { new(...args: any[]): T }): T[] {
  ...
  _getImageTracks(): Array<PKImageTrack> {
    return this._getTracksByType<PKImageTrack>(PKImageTrack);
  }
  ...
  _getVideoTracks(): Array<PKVideoTrack> {
    return this._getTracksByType<PKVideoTrack>(PKVideoTrack);
  }
   ...
   * @private
   */
  _getAudioTracks(): Array<PKAudioTrack> {
    return this._getTracksByType<PKAudioTrack>(PKAudioTrack);
  }
  ...
  _setDefaultTracks(): void {
    ...
    const offTextTrack: PKTrack = this._getTextTracks().find(track => PKTextTrack.langComparer(OFF, track.language))! ;
    ...
    const currentOrConfiguredAudioLang =
      ...
      this._getLanguage<PKAudioTrack>(this._getAudioTracks(), playbackConfig.audioLanguage, activeTracks.audio);
    ...
    if (currentOrConfiguredAudioLang === playbackConfig.audioLanguage) {
      this._setDefaultTrack<PKAudioTrack>(
      ...
      this._setDefaultTrack<PKAudioTrack>(this._getAudioTracks(), currentOrConfiguredAudioLang, activeTracks.audio);
    }
   ...
   */
  _getLanguage<T extends PKTextTrack | PKAudioTrack>(tracks: T[], configuredLanguage: string, defaultTrack?: T): string {
    let language = configuredLanguage;
    if (language === AUTO) {
      const localeTrack: T | undefined = tracks.find(track => PKTrack.langComparer(Locale.language, track.language));
      if (localeTrack) {
   ...
   * @private
   */
  _setDefaultTrack<T extends PKTextTrack | PKAudioTrack>(
    tracks: T[],
    language: string,
    defaultTrack?: PKTrack | null,
    ...
    const sameTrack: T | undefined = tracks.find(track => PKTrack.langComparer(language, track.language, additionalLanguage, true));
    if (sameTrack) {
      updateTrack(sameTrack);
    } else {
      const track: T | undefined = tracks.find(track => PKTrack.langComparer(language, track.language, additionalLanguage, false));
      if (track) {
        updateTrack(track);
  ...
  _setDefaultVideoTrack(): void {
    const sortedVideoTracks = this._getVideoTracks().sort((track1: PKVideoTrack, track2: PKVideoTrack) => track2.bandwidth - track1.bandwidth);
    let selectedVideoTrack = sortedVideoTracks.find(
      (track: PKVideoTrack) => track.label && track.label === this._playbackAttributesState.videoTrack?.label
    );
    if (!selectedVideoTrack) {
      selectedVideoTrack = sortedVideoTracks.find(
        (track: PKVideoTrack) => track.height && track.height === this._playbackAttributesState.videoTrack?.height
      );
    }
    if (selectedVideoTrack) {
   ...
   * @returns {void}
   */
  _setTracksCustomLabels<T extends PKAudioTrack | PKTextTrack | PKVideoTrack>(tracks: T[], callback: (track: T) => string): void {
