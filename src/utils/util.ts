...
const _Object = {
   ...
   * @returns {Object} - The merged object.
   */
  mergeDeep: function (target: any, ...sources: any): any {
    if (!sources.length) {
      return target;
    }
  ...
  defer: function (): DeferredPromise {
    ...
    promise['resolve'] = res;
    promise['reject'] = rej;
    return promise as unknown as DeferredPromise;
  },
...
const _Dom = {
   ...
   * @returns {Element} - The first element that matches a specified CSS selector(s) in the document.
   */
  insertBefore(parent: HTMLElement, newChild: HTMLElement, existingChild: HTMLElement | null = null): HTMLElement | null {
    try {
...
const _VERSION = {
  ...
  compare: function (v1: string, v2: string, options: any = {}) {
    ...
    if (!lexicographical) {
      v1parts = mapParts(v1parts as string[]);
      v2parts = mapParts(v2parts as string[]);
    }
