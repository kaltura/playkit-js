...
export default class NativeAdapter extends BaseMediaSourceAdapter {
   ...
   * @static
   */
  static _drmProtocol: IDrmProtocol | null = null;
  /**
   ...
   * @private
   */
  _drmHandler: FairPlayDrmHandler | undefined;
  /**
   ...
   * @private
   */
  _heartbeatTimeoutId: number | undefined;
  /**
   ...
   * @private
   */
  _videoDimensions: PKVideoDimensionsObject | undefined;

  _loadPromiseReject: ((error: Error) => any) | undefined;
  ...
  load(startTime?: number): Promise<{tracks: PKTrack[]}> {
    ...
    if (!this._loadPromise) {
      this._loadPromise = new Promise<{tracks: PKTrack[]}>((resolve, reject) => {
   ...
   * @returns {void}
   */
  _onLoadedData(resolve: (value: (PromiseLike<{tracks: PKTrack[]}> | {tracks: PKTrack[]})) => void, startTime?: number): void {
  ...
  _handleVideoTracksChange(): void {
    if (!this._isProgressivePlayback()) {
      ...
      if (!this._videoDimensions || videoHeight !== this._videoDimensions.videoHeight || videoWidth !== this._videoDimensions.videoWidth) {
        ...
        this._onTrackChanged(new PKVideoTrack(setting));
      }
    }
  }
   ...
   * @private
   */
  _getParsedTracks(): Array<PKTrack> {
   ...
   * @private
   */
  _getParsedVideoTracks(): Array<PKVideoTrack> {
    if (this._isProgressivePlayback()) {
   ...
   * @private
   */
  _getParsedProgressiveVideoTracks(): Array<PKVideoTrack> {
    const videoTracks = this._progressiveSources;
    const parsedTracks: PKVideoTrack[] = [];
    if (videoTracks) {
      for (let i = 0; i < videoTracks.length; i++) {
        ...
        parsedTracks.push(new PKVideoTrack(settings));
      }
    }
    return parsedTracks;
  }
   ...
   * @private
   */
  _getParsedAdaptiveVideoTracks(): Array<PKVideoTrack> {
   ...
   * @private
   */
  _getParsedAudioTracks(): Array<PKAudioTrack> {
    // TODO - why it is not on the type ???
    // eslint-disable-next-line  @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const audioTracks = this._videoElement.audioTracks;
    const parsedTracks: PKAudioTrack[] = [];
    if (audioTracks) {
      for (let i = 0; i < audioTracks.length; i++) {
        ...
        parsedTracks.push(new PKAudioTrack(settings));
      }
    }
    return parsedTracks;
  }
   ...
   * @public
   */
  _selectProgressiveVideoTrack(videoTrack: PKVideoTrack): void {
    let videoTracks = this._progressiveSources;
    if (videoTrack instanceof PKVideoTrack && videoTracks && videoTracks[videoTrack.index]) {
   ...
   * @public
   */
  selectAdaptiveVideoTrack(videoTrack: PKVideoTrack): void {
    // TODO - why it is not on the type ???
    // eslint-disable-next-line  @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const videoTracks = this._videoElement.videoTracks;
  ...
  selectAudioTrack(audioTrack: PKAudioTrack): void {
    ...
    // @ts-ignore
    const audioTracks = this._videoElement.audioTracks;
    if (audioTrack instanceof PKAudioTrack && audioTracks && audioTracks[audioTrack.index]) {
  ...
  _getPKAudioTracks(): Array<PKAudioTrack> {
    const audioTracks = this._playerTracks.filter(track => track instanceof PKAudioTrack);
    return audioTracks;
  }

  _getActivePKAudioTrack(): PKAudioTrack | undefined {
  ...
  _maybeApplyAbrRestrictions(restrictions: PKABRRestrictionObject): void {
    if (this._isProgressivePlayback()) {
      ...
      // eslint-disable-next-line  @typescript-eslint/ban-ts-comment
      // @ts-ignore
      const videoTracks = this._playerTracks.filter(track => track instanceof PKVideoTrack);
      const availableTracks = filterTracksByRestriction(videoTracks as PKVideoTrack[], restrictions);
