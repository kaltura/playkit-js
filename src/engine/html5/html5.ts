...
export default class Html5 extends FakeEventTarget implements IEngine {
  ...
  load(startTime?: number): Promise<{tracks: PKTrack[]}> {
    ...
    return this._canLoadMediaSourceAdapterPromise
      .then(() => {
        return this._mediaSourceAdapter ? this._mediaSourceAdapter.load(startTime) : Promise.resolve({} as {tracks: PKTrack[]});
      })
      .catch(error => {
   ...
   */
  set crossOrigin(crossOrigin: string | undefined) {
      ...
      this._eventManager.listen(metadataTrack, 'cuechange', () => {
        ...
        Array.from(this._el.textTracks).forEach((track: TextTrack) => {
          if (PKTextTrack.isMetaDataTrack(track)) {
            // @ts-ignore
            activeCues = activeCues.concat(track.activeCues);
          }
        });
